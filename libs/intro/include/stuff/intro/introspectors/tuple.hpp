...
public:
    ...
    template<usize I, typename Tuple>
        requires(std::is_same_v<std::decay_t<Tuple>, std::tuple<Ts...>> ||
                 std::is_same_v<std::decay_t<Tuple>, pair_type>)
    static constexpr auto get(Tuple&& v) -> decltype(auto) {
        return std::get<I>(std::forward<Tuple>(v));
    }

    template<typename Fn, typename Tuple>
        requires(std::is_same_v<std::decay_t<Tuple>, std::tuple<Ts...>> || std::is_same_v<std::decay_t<Tuple>, pair_type>)
    static constexpr void iterate(Tuple&& v, Fn&& fn) {
        return iterate_impl<Fn, Tuple>(std::forward<Tuple>(v), std::forward<Fn>(fn));
    }

private:
    template<typename Fn, typename Tuple, usize I = 0>
    static constexpr void iterate_impl(Tuple&& v, Fn&& fn) {
        constexpr bool is_lvalue = std::is_lvalue_reference_v<Tuple>;
        using elem_type = std::tuple_element_t<I, std::decay_t<Tuple>>;
        ...
        if constexpr (constexpr_iterno_functor) {
            if constexpr (is_lvalue) {
                std::invoke(std::forward<Fn>(fn), std::integral_constant<usize, I>{}, std::get<I>(v));
            } else {
                std::invoke(
                  std::forward<Fn>(fn), std::integral_constant<usize, I>{},
                  std::move(std::get<I>(std::forward<Tuple>(v)))
                );
            }
        } else if constexpr (runtime_iterno_functor) {
            if constexpr (std::is_lvalue_reference_v<Tuple>) {
                std::invoke(std::forward<Fn>(fn), I, std::get<I>(v));
            } else {
                std::invoke(std::forward<Fn>(fn), I, std::move(std::get<I>(std::forward<Tuple>(v))));
            }
        }

        if constexpr (I + 1 != size()) {
            return iterate_impl<Fn, Tuple, I + 1>(std::forward<Tuple>(v), std::forward<Fn>(fn));
        }
    }
};
...
constexpr auto _stf_adl_introspector(tuple<Ts...> const&) {
    return ::stf::intro::detail::tuple_introspector<Ts...>{};
}
