...
struct binary_stream {
    ...
    constexpr auto next() -> std::optional<triangle> {
        ...
        if (
          !read_primitives<float>({ret.normal}) ||
          !read_primitives<float>({ret.vertices[0]}) ||
          !read_primitives<float>({ret.vertices[1]}) ||
          !read_primitives<float>({ret.vertices[2]})) {
            return std::nullopt;
        }
